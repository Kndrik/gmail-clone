...
import { getEmailFromId } from "../EmailFetcher";
...
const MailView = (props) => {
    ...
    const getEmail = async () => {
        let result = await getEmailFromId(mailId);
        console.log(result);
        setEmail(result);
        setLoading(false);
    }
    ...
    return (
        ...
        <div className="mailView">
            <div className="mailViewHeader">
                <div className="left">
                    <h2 className="mailViewTitle">{email.title}</h2>
                </div>
                <div className="right">
            ...
            <div className="mailViewContent">
                <div className="left">
                    <div className="photoContainer">
                        <img className="photo" src={email.senderPicURL} />
                    </div>
                </div>
                <div className="right">
                    <div className="mailViewInformations">
                        <div className="top">
                            <div className="left">
                                <span className="mailViewSenderName">{email.sender}</span>
                                <span className="mailViewSenderAddress">{"<"+(email.senderEmail)+">"}</span>
                            </div>
                            <div className="right">
                                <span className="mailViewDate">{email.date ? email.date.toDate().toLocaleDateString() : ""}</span>
                        ...
                        <div className="bottom">
                            <span className="mailViewReceiver">{"To " + email.receiver }</span>
                        </div>
                    </div>
